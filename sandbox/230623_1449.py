...
class View(ui.View):
  ...
  def plot_radius_index(self, s_cell: ui.Path, e_cell: ui.Path):
    ...
    #x, y = self.get_position_to_index(cx, cy)
    #indexs = [x - self.cell_rad, y-self.cell_rad]
    indexs = self.get_position_to_index(cx, cy)
    return indexs
  ...
  def draw(self):
    ...
    for n, adrs in enumerate(self.rect_edge_index):
      ...
      #self.oblique_length = self.get_oblique_length(s_cell, e_cell)
      ...
      self.create_line_cells_index(s_cell, e_cell, hsv_color)
